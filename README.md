...
- [Writing a Restfine API](#writing-a-Restfine-api)
...
- [Consuming a Restfine API](#consuming-a-Restfine-api)
...
- [Upgrading Restfine](#upgrading-Restfine)
...
like to create an API with Restfine (keep scrolling for a JavaScript version):
...
# Writing A Restfine API

**Restfine is a _server-only_ package. Attempting to access any of its methods from the client will
result in an error.**

## Configuration Options

The following configuration options are available when initializing an API using
`new Restfine(options)`:
  ...
  - `user`  _Function_
    ...
    - Provides one of two levels of authentication, depending on the data returned. The context
      ...
      - Partial auth
        ...
        - If both a `userId` and `token` are returned, Restfine will hash the token, then,
...
Well, you're in luck, because this is almost _too easy_ with Restfine! All available REST endpoints
(except `patch` and `options`, for now) can be generated for a Mongo Collection using
`Restfine#addCollection()`. This generates two routes by default:
...
The first - and only required - parameter of `Restfine#addCollection()` is a Mongo Collection.
...
Route and endpoint configuration options are available in `Restfine#addCollection()` (as the 2nd,
optional parameter).
...
All responses generated by Restfine follow the [JSend] format, with one minor tweak: failures have
...
Routes are defined using `Restfine#addRoute()`. A route consists of a path and a set of endpoints
defined at that path.

### Path Structure

The `path` is the 1st parameter of `Restfine#addRoute`. You can pass it a string or regex. If you
...
The following options are available in `Restfine#addRoute` (as the 2nd, optional parameter):
##### `authRequired`
- _Boolean_
- Default: `false`
...
The last parameter of `Restfine#addRoute` is an object with properties corresponding to the supported
HTTP methods. At least one method must have an endpoint defined on it. The following endpoints can
be defined in Restfine:
- `get`
- `post`
- `put`
- `patch`
...
- Default:
  ...
  - `Access-Control-Allow-Origin`: `*`
    - This is a [CORS-compliant header][cors] that allows requests to be made to the API from any
      ...
      [disabled by default](https://github.com/kahmali/meteor-Restfine#enablecors), or also by
...
Currently, there is only a single versioning strategy supported in Restfine: URL path versioning. In
...
package](http://docs.meteor.com/#/full/accounts_passwords). You can do this with Restfine if you
...
- `user`: **Note: This is for legacy purposes only. It is recommended to use one of the options
  above.** Accepts either of the options listed above. Restfine will (very naively) attempt to
...
To update Restfine to the latest version:
```bash
> meteor update nimble:Restfine
```

Or to update Restfine to a specific version:
```bash
> meteor add nimble:Restfine@=<version_number>
```

For example, to update Restfine to v0.7.0:
```bash
> meteor add nimble:Restfine@=0.7.0
```

Please check the [change log][Restfine-change-log] before updating, for more information about the
changes between each version. More detailed instructions for updating between major versions are
included below.

## Upgrading to 0.8.0

The most noticeable difference in v0.8.0 is that Restfine is now exported as a "class" instead of an
object. API configuration has also been moved from `Restfine.configure()` (which has been removed),
to the Restfine constructor. This means that instead of being forced to configure a single Restfine
API with
...
Restfine.addRoute('example');
```

you can configure as many separate APIs as you need with

```javascript
FirstApi = new Restfine({
  apiPath: 'first-api',
  ...
});

SecondApi = new Restfine({
  apiPath: 'second-api',
  ...
});
...
replaced by [`simple:json-routes`][json-routes] as the server-side router for Restfine. This means
that Restfine should no longer [interfere with other routers]
(https://github.com/kahmali/meteor-Restfine/issues/24) (client _or_ server), or do other [annoying]
(https://github.com/kahmali/meteor-Restfine/issues/35) [things]
(https://github.com/kahmali/meteor-Restfine/issues/43). Special thanks to [Sashko Stubailo]
(https://github.com/stubailo) for his work on `simple:json-routes`, and for handling the conversion
from `iron:router` to `simple:json-routes` in Restfine!
...
(https://github.com/kahmali/meteor-Restfine/blob/devel/CHANGELOG.md#v080---2015-07-06).
...
Restfine used to store the account login token in the `Meteor.user` document at
...
that all clients using the default authentication in a Restfine API will need to reauthenticate with
their username/email and password after this update, as their existing tokens will be rendered
invalid.

## Upgrading to 0.6.1

Restfine v0.6.1 brings support for easily generating REST endpoints for your Mongo Collections, and
with that comes a few API-breaking changes:

- The `Restfine.add()` method has been changed to `Restfine.addRoute()` for clarity, now that the
  `Restfine.addCollection()` method has been added.
- All responses generated by Restfine now follow the [JSend] format, with one minor tweak: failures
  ...
  this only affects responses that are automatically generated by Restfine. Any responses you
...
[Restfine Swagger](https://github.com/apinf/Restfine-swagger)
- Define and generate [Swagger 2.0](http://swagger.io/) documentation for Restfine API

## Change Log

A detailed list of the changes between versions can be found in the [change log]
(https://github.com/kahmali/meteor-Restfine/blob/master/CHANGELOG.md).

## Contributing

Contributions to Restfine are welcome and appreciated! If you're interested in contributing, please
check out [the guidelines](https://github.com/kahmali/meteor-Restfine/blob/master/CONTRIBUTING.md)
...
work on [`simple:json-routes`][json-routes], including the Restfine conversion from [Iron
Router][iron-router].
...
MIT License. See [LICENSE](https://github.com/kahmali/meteor-restfine/blob/master/LICENSE) for
details.
...
[Restfine-change-log]:  https://github.com/kahmali/meteor-Restfine/blob/master/CHANGELOG.md       "Restfine Change Log"
[Restfine-issues]:      https://github.com/kahmali/meteor-Restfine/issues                         "Restfine Issues"
